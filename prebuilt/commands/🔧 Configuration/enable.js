...
module.exports = {
  ...
  variables: {
    ...
    description: {
      ...
      description: "The description of the response embed. Use {feature} to show the feature that has been enabled.",
      default: "The feature `{feature}` has been enabled."
    },
  },
  ...
  command: async ({
    ...
    if ((commandType(event) === "message") && !event.content.split(" ").slice(1).join(" ")) return event.reject("Please provide a feature to enable.");
    ...
    event.store.disabledFeatures = event.store.disabledFeatures.filter((feature) => feature !== ((commandType(event) === "message") ? event.content.split(" ").slice(1).join(" ") : event.options.getString("feature")).toLowerCase());
    if (!event.store.disabledFeatures.length) delete event.store.disabledFeatures;
  ...
  slashCommand: (SlashCommandBuilder) ? (
    new SlashCommandBuilder()
      .addStringOption((option) =>
        option
          .setName("feature")
          .setDescription("Feature to enable (e.g. `all`, `music`, `fun`, etc.)")
          .setRequired(true)
          .addChoices(
            ...[
              ...[
